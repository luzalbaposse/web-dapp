...
RSpec.describe "Invited talents API", type: :request do
  ...
  path "/invited_talents" do
    get "Retrieves a list of invited talents" do
      ...
      let!(:invited_user_one) { create :user, invited: invite, created_at: ExperiencePoints::CreditInvitePoints::START_DATE + 1 }
      let!(:talent_one) { create :talent, user: invited_user_one, verified: true }
      let!(:invited_user_two) { create :user, invited: invite, created_at: ExperiencePoints::CreditInvitePoints::START_DATE - 20 }
      let!(:talent_two) { create :talent, user: invited_user_two }
      let!(:token_two) { create :talent_token, talent: talent_two, deployed: true, deployed_at: ExperiencePoints::CreditInvitePoints::START_DATE - 5 }
      let!(:invited_user_three) { create :user, invited: invite, created_at: ExperiencePoints::CreditInvitePoints::START_DATE + 4 }
      let!(:talent_three) { create :talent, user: invited_user_three, verified: false }
      let!(:invited_user_four) { create :user, invited: invite, created_at: ExperiencePoints::CreditInvitePoints::START_DATE - 4 }
      let!(:talent_four) { create :talent, user: invited_user_four, verified: true }
      let!(:token_four) { create :talent_token, talent: talent_four, deployed: true, deployed_at: ExperiencePoints::CreditInvitePoints::START_DATE + 4 }
      let!(:invited_user_five) { create :user, invited: invite, created_at: ExperiencePoints::CreditInvitePoints::START_DATE + 6 }
      ...
      response "200", "Invited talents retrieved", document: false do
        ...
        run_test! do |response|
          ...
          aggregate_failures do
            expect(returned_talent_one["experience_points_amount"]).to eq(ExperiencePoints::CreditInvitePoints::AMOUNT)
            ...
            expect(returned_talent_two["experience_points_amount"]).to eq(0)
            ...
            expect(returned_talent_three["experience_points_amount"]).to eq(0)
            ...
            expect(returned_talent_four["experience_points_amount"]).to eq(0)
            ...
            expect(returned_talent_five["experience_points_amount"]).to eq(0)
