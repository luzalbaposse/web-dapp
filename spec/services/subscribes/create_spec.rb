...
RSpec.describe Subscribes::Create do
  include ActiveJob::TestHelper

  subject(:create_subscribe) { described_class.new(subscribing_user: subscribing_user, subscriber_user: subscriber_user).call }

  let(:subscriber_user) { create :user }
  let(:subscribing_user) { create :user }

  it "creates a new subscribe" do
    expect { create_subscribe }.to change(Subscribe, :count).from(0).to(1)
  end

  it "creates two connections" do
    expect { create_subscribe }.to change(Connection, :count).from(0).to(2)
  end

  it "creates the subscribe with the correct arguments" do
    created_subscribe = create_subscribe
  ...
  context "when the subscribe already exists" do
    before { create :subscribe, subscriber: subscriber_user, user: subscribing_user }

    it "raises an error" do
      expect { create_subscribe }.to raise_error(described_class::AlreadyExistsError)
    end
  end
  ...
  context "when the user has more than 2 follows" do
    before do
      create :subscribe, user: create(:user), subscriber: subscriber_user
      create :subscribe, user: create(:user), subscriber: subscriber_user
    end
