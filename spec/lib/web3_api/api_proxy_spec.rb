...
RSpec.shared_examples "a moralis client get wallet nfts request" do
  let(:client_class) { Web3Api::Moralis::Client }
  let(:client) { instance_double(client_class) }
  ...
  let(:response) do
    OpenStruct.new(
      success?: response_success,
      body: file_fixture("moralis_get_wallet_nfts_response.json").read
    )
  end
  let(:response_success) { true }
  ...
  it "returns an array with the wallet NFTs" do
...
RSpec.describe Web3Api::ApiProxy do
  ...
  let(:contract_addresses) { [SecureRandom.hex, SecureRandom.hex] }
  ...
  describe "#retrieve_wallet_nfts" do
    subject(:request) { api_proxy.retrieve_wallet_nfts(wallet_address: wallet_address, chain: chain, contract_addresses: contract_addresses) }
    ...
    context "when the chain matches ethereum network" do
      context "when the chain is eth" do
        ...
        it_behaves_like "a moralis client get wallet nfts request"
      end
    ...
    context "when the chain matches the bsc network" do
      let(:chain) { "bsc" }

      context "when the chain is bsc" do
        let(:chain) { "bsc" }
        let(:formatted_chain) { "bsc" }

        it_behaves_like "a moralis client get wallet nfts request"
      end

      context "when the chain is 0x38" do
        let(:chain) { "0x38" }
        let(:formatted_chain) { "0x38" }

        it_behaves_like "a moralis client get wallet nfts request"
      end

      context "when the chain is 56" do
        let(:chain) { 56 }
        let(:formatted_chain) { "0x38" }
