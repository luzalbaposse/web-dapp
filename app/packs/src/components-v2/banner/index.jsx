...
export const Banner = ({ bannerUrl, deleteBannerCallback, canUpdate, isUploading }) => {
  ...
  const updateActions = useMemo(
    () => (
      <>
        <input id="overviewBannerFileInput" className="d-none" type="file" accept=".jpg,.png,.jpeg,.gif" />
        <EditImagePositioner className="edit-image banner-profile">
          <StyledLabel htmlFor="overviewBannerFileInput">
            <TalentProfilePicture className="cursor-pointer" src={cameraButtonImage} height={40} />
          </StyledLabel>
          <StyledButton className="button-link" onClick={deleteBannerCallback}>
            <TalentProfilePicture className="cursor-pointer" src={deleteButtonImage} height={40} />
          </StyledButton>
        </EditImagePositioner>
      </>
    ),
