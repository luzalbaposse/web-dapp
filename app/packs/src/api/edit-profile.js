...
const editProfile = (username, profile) => {
  ...
  return axios.patch(
    `/api/v1/talents/update_profile?id=${username}`,
    { ...profile },
    {
      headers: {
        ...headers
...
const editAbout = (username, profile) => {
  ...
  const headers = appendCSRFToken(baseHeaders);
  return axios.patch(
    `/api/v1/talents/update_about?id=${username}`,
    { ...profile },
    {
      headers: {
        ...headers
...
const editExperience = (username, profile) => {
  ...
  const headers = appendCSRFToken(baseHeaders);
  return axios.patch(
    `/api/v1/talents/update_experience?id=${username}`,
    { ...profile },
    {
      headers: {
        ...headers
...
const editAccount = (username, profile) => {
  ...
  const headers = appendCSRFToken(baseHeaders);
  return axios.patch(
    `/api/v1/talents/update_account?id=${username}`,
    { ...profile },
    {
      headers: {
        ...headers
