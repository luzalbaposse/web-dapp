      ...
      <Table.Body>
        {connections.map((connection) => (
          <Table.Tr
            key={connection.id}
            onClick={() =>
              (window.location.href = `/profiles/${connection.username}`)
            }
          >
            <Table.Td>
              <div className="d-flex align-items-center">
                ...
                {connection.ticker && (
                  <P2
                    ...
                    style={{
                      color:
                        mode == "dark" ? darkTextPrimary03 : lightTextPrimary03,
                    }}
                  />
                )}
              </div>
            ...
            <Table.Td>
              <Tag
                className={cx(
                  "connection",
                  `connection__${connection.connection_type}`,
                  mobile ? "ml-auto" : ""
                )}
              >
                <div className="d-flex align-items-center">
                  ...
                  <P2
                    mode={mode}
                    text={formattedConnectionType(connection.connection_type)}
                    bold
                    role="button"
                  />
                </div>
            ...
            <Table.Td className="hide-content-in-mobile">
              <P2>
                {connection.user_invested_amount > 0 && (
                  <>
                    <span className="bold">
                      {displayableAmount(connection.user_invested_amount)}
                    </span>
                ...
                {connection.connected_user_invested_amount > 0 && (
                  <>
                    <span className="bold">
                      {displayableAmount(
                        connection.connected_user_invested_amount
                      )}
                    </span>
            ...
            <Table.Td className="hide-content-in-mobile">
              <P2
                text={dayjs(connection.connected_at, "YYYY-MM-DD").format(
                  "MMM DD, YYYY"
                )}
              />
            </Table.Td>
...
const SearchForm = ({
  ...
  const selectedClass = (option) =>
    option.name == connectionType.name ? " text-primary" : "text-black";
...
const Community = ({ userId, talent, canUpdate }) => {
  ...
  const [keyword, setKeyword] = useState(
    url.searchParams.get("community_q") || ""
  );

  useEffect(() => {
    const params = new URLSearchParams(document.location.search);

    get(`/api/v1/users/${userId}/profile/community?${params.toString()}`).then(
      (response) => {
        if (response.error) {
  ...
  const debouncedSetParamsAndSearch = debounce(
    (field, option) => setParamsAndSearch(field, option),
    500
  );
