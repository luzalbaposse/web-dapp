...
export function drawHover(context, data, settings) {
  ...
  context.font = `${weight} ${connectionTypeLabelSize}px ${font}`;
  const coinsLabelWidth = coinsLabel ? context.measureText(coinsLabel).width : 0;

  const textWidth = Math.max(labelWidth, connectionTypeLabelWidth, coinsLabelWidth);
  ...
  const hcoinsLabel = Math.round(connectionTypeLabelSize / 2 + 9);

  drawRoundRect(context, x, y - hconnectionTypeLabel - 12, w, hcoinsLabel + hLabel + hconnectionTypeLabel + 12, 5);
  context.closePath();
  context.fill();
  ...
  context.fillStyle = data.color;
  context.font = `${weight} ${connectionTypeLabelSize}px ${font}`;
  context.fillText(coinsLabel, data.x + data.size + 3, data.y + size / 3 + 3 + connectionTypeLabelSize);
}
