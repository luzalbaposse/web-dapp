...
const Nfts = ({ user, canUpdate, setShowLastDivider, onWalletConnect, railsContext }) => {
  ...
  useEffect(() => {
    ...
    get(`/api/v1/users/${userId}/profile/web3/nfts?visible=${true}`).then(response => {
      if (response.error) {
  ...
  const removeNft = (previousNfts, updatedNft) => {
    ...
    const newNfts = [...previousNfts.slice(0, nftIndex), ...previousNfts.slice(nftIndex + 1)];
  ...
  const loadMoreNfts = () => {
    ...
    get(`/api/v1/users/${userId}/profile/web3/nfts?page=${nextPage}&visible=${true}`).then(response => {
      mergeNfts(response.tokens);
      setPagination(response.pagination);
    });
  };
  ...
  return (
    <section className="d-flex flex-column align-items-center my-7">
      ...
      <div className="container">
        ...
        <P1 className="text-center pb-3 mb-4">A curated list of my main nfts</P1>
        {(nfts.length == 0 || !walletConnected) && canUpdate && (
          <>
            <H5 bold text={"You don't have any NFTs to showcase"} className="text-primary-01 text-center mb-2 mt-7" />
            <P2
              text={
              ...
              <div className="col-12 col-md-4 mb-4" key={nft.id}>
                <div className="card web3-card">
                  <div className="mb-3 d-flex flex-column justify-content-between">
                    ...
                    <P3
                      text={nft.description?.length > 150 ? `${nft.description.substring(0, 150)}...` : nft.description}
        ...
        {showLoadMoreNfts() && (
          <div className="d-flex flex-column justify-content-center mt-2">
            <ThemedButton onClick={() => loadMoreNfts()} type="white-subtle" className="mx-auto">
              Show More
            </ThemedButton>
          </div>
        )}
