...
const MobileSupportingDropdown = ({ show, hide, mode, selectedOption, order, onOptionClick }) => {
...
const Supporting = ({
  ...
  useEffect(() => {
    get(`api/v1/public_talent/${concatenateSupportingAddresses(talents)}`).then(response => {
  ...
  const compareRewards = (talent1, talent2) => {
    const talent1Amount = parseFloat(returns(talent1.contract_id).replaceAll(",", ""));
    const talent2Amount = parseFloat(returns(talent2.contract_id).replaceAll(",", ""));
  ...
  if (sortedTalents().length == 0) {
    return (
      <div className="w-100 h-100 d-flex flex-column justify-content-center align-items-center mt-3">
        ...
        <P2 mode={mode} text="All your talent tokens will be listed here" bold />
        <Button onClick={() => (window.location.href = "/talent")} type="primary-default" mode={mode} className="mt-3">
          See talent
        </Button>
      </div>
    );
      ...
      <>
        ...
        <div className="d-flex flex-column w-100 mt-3 px-4">
          ...
          <div className="d-flex flex-row w-100 align-items-end mt-3">
            <H4 mode={mode} text={currency(talentTokensInCUSD).format()} bold className="mb-0" />
            <P2 mode={mode} text={`${currency(talentTokensInTAL).format().substring(1)} $TAL`} className="ml-2" />
          </div>
        </div>
                ...
                <Table.Td>
                  <div className="d-flex cursor-pointer pl-4 py-2">
                    <TalentProfilePicture
                      ...
                      userId={talent.id}
                    />
                    <P2 text={`${talentProfileUsernames[talent.contract_id]}`} bold className="ml-2" />
                  </div>
                </Table.Td>
                <Table.Td className="d-flex flex-column justify-content-center align-items-end pr-4 py-2">
                  <P2 bold text={getSelectedOptionValueUSD(talent)} className="text-black" />
  ...
  return (
    <>
      <Table mode={mode} className="px-3 horizontal-scroll mt-4">
        ...
        <Table.Body>
          {sortedTalents().map(talent => (
            <Table.Tr key={`talent-${talent.contract_id}`} className="reset-cursor">
              <Table.Td>
                <div
                  className="d-flex cursor-pointer"
                  onClick={() => (window.location.href = `/u/${talentProfileUsernames[talent.contract_id]}`)}
                >
                  <TalentProfilePicture
                    src={talentProfilePictures[talent.contract_id]}
                    userId={talent.id}
              ...
              <Table.Td>
                <div className="d-flex flex-column justify-content-center align-items-start">
                  <P2 bold text={`$${parseAndCommify(talent.amount * 0.1)}`} className="text-black" />
                  <P2 text={`${parseAndCommify(talent.amount)} ${talent.symbol}`} />
                </div>
              </Table.Td>
              <Table.Td>
                <div className="d-flex flex-column justify-content-center align-items-start">
