...
const ClaimRewardsDropdown = ({ className, mode, talentSymbol }) => {
  const [selectedItem, setSelectedItem] = useState(`Use my rewards to buy more $${talentSymbol}`);
  ...
  return (
    <Dropdown>
      ...
      <Dropdown.Menu className={cx("w-100", mode)}>
        <Dropdown.Item key="tab-dropdown-restake" onClick={e => setSelectedItem(e.target.innerText)}>
          <P3 className="text-black" bold mode={mode} text={`Use my rewards to buy more $${talentSymbol}`} />
        </Dropdown.Item>
        <Dropdown.Item key="tab-dropdown-withdrawal" className="d-flex align-items-center" disabled>
          <P3 className="text-primary-04" bold mode={mode} text="Claim Rewards to my wallet" />
