module Users
  class Create
    ...
    def create_subscribe(invite, user)
      return unless invite

      invited_by_user = invite.user
      return unless invited_by_user

      subscribe = Subscribe.find_or_create_by(user_id: user.id, subscriber_id: invited_by_user.id)

      unless subscribe.persisted? # validate if the watchlist quest is completed
