    ...
    def reward_winners
      ...
      ActiveRecord::Base.transaction do
        if winners[0].present?
          ...
          Reward.create!(user: first_winner, amount: 500, category: "race")
        end
        if winners[1].present?
          second_winner = User.find_by(id: winners[1]["id"])
          Reward.create!(user: second_winner, amount: 300, category: "race")
        end
        if winners[2].present?
          third_winner = User.find_by(id: winners[2]["id"])
          Reward.create!(user: third_winner, amount: 200, category: "race")
        end
      end
    end
  end
