class Connection < ApplicationRecord
  ...
  def calculate_connection_type(user_invested_amount, connected_user_invested_amount)
    ...
    return 3 if user.users_subscribing.find_by(user_id: connected_user_id).present? && connected_user.users_subscribing.find_by(user_id: user_id).present?
    return 2 if user.users_subscribing.find_by(user_id: connected_user_id).present?
    return 1 if connected_user.users_subscribing.find_by(user_id: user_id).present?
  end
end
